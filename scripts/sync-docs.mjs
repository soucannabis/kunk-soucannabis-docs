#!/usr/bin/env node
/**
 * Copia docs-source/ → src/content/docs (api, frontend, funcionalidades)
 * para o site público Starlight.
 */
...
const SRC = path.join(WEBSITE_ROOT, "docs-source");
const DEST = path.join(WEBSITE_ROOT, "src/content/docs");

const SYNC_DIRS = ["api", "frontend", "funcionalidades"];

/**
 * Pasta em docs-source → pasta publicada no Starlight (URL pública).
 * `api` vira `referencia-api` para não colidir com o proxy nginx `/api/v1`
 * nem com CDNs que encaminham `/api/*` à kunk-api.
 */
const DEST_DIR_MAP = {
  api: "referencia-api",
};

/** Paths relativos a docs-source que não entram no site. */
const SKIP_REL = new Set([]);

function mapDestRel(rel) {
  const parts = rel.split("/");
  const root = parts[0];
  if (DEST_DIR_MAP[root]) {
    parts[0] = DEST_DIR_MAP[root];
  }
  return parts.join("/");
}
...
function scrubLegacy(body) {
  ...
  // Seções inteiras cujo título cita ferramentas/legado antigo / fonte legada / placeholders
  text = text.replace(
    /^#{2,6}[^\n]*\b(Directus|DocuSeal|kunkserver|legado|v1|Fonte legada|Placeholders)\b[^\n]*\n(?:.*\n)*?(?=^#{2,6} |\Z)/gim,
    ""
  );

  // Integrações legadas — nunca publicar no site
  text = text.replace(/^.*\bbeeviral\b.*\n?/gim, "");
  text = text.replace(/\bBeeviral\b/gi, "");
  text = text.replace(/\bbeeviral\b/gi, "");
  text = text.replace(/^.*\bpipefy\b.*\n?/gim, "");
  text = text.replace(/\bpipefy\b/gi, "");
  text = text.replace(/^.*\bbrasilnfe\b.*\n?/gim, "");
  text = text.replace(/\bbrasilnfe\b/gi, "");
  text = text.replace(/`nibo`/gi, "");
  text = text.replace(/^- `nibo`\s*$/gim, "");
  text = text.replace(/^- `scp`\s*$/gim, "");
...
function toStarlightMarkdown(raw, fallbackTitle) {
  ...
  if (h1) {
    // Não remover `_` (ids tipo system_api_credentials) — só ênfase markdown.
    title = h1[1]
      .replace(/\[([^\]]+)\]\([^)]+\)/g, "$1")
      .replace(/[*`]/g, "")
      .trim();
...
function pruneOrphans(keep) {
  ...
  // Pasta antiga /api (docs) — removida após migração para referencia-api
  fs.rmSync(path.join(DEST, "api"), { recursive: true, force: true });
  const destRoots = new Set(
    SYNC_DIRS.map((d) => DEST_DIR_MAP[d] || d).concat(SYNC_DIRS)
  );
  for (const dir of destRoots) {
...
function sync() {
  ...
  for (const dir of SYNC_DIRS) {
    ...
    for (const file of walkMarkdown(fromRoot)) {
      ...
      let outRel = mapDestRel(rel);
      if (path.basename(outRel) === "README.md") {
        outRel = path.join(path.dirname(outRel), "index.md");
      }
  ...
  console.log(
    `sync-docs: ${count} páginas → ${path.relative(WEBSITE_ROOT, DEST)} (escritas: ${written}, omitidas: ${skipped})`
  );
}
