...
Nomes = tabelas do schema do produto (`kunk-api`).
...
| `users_api` | `token` (não expor em plaintext) |
...
### `tags`
- `contexts` — escopos: `orders`, `services`, `reception`

### Junctions `*_files`
- `order_id` / `user_id` / `service_id` + `file_id`
- Preferir endpoints `/files` + attach, em vez de CRUD cru na junction (ambos podem coexistir)

## Collections **não** disponíveis neste produto

Não entram na whitelist (schemas antigos fora do produto atual):

`Coupons`, `Deliveries`, `Satisfaction_survey`, `batch_control`, `changelog`, `finances`, `logs`, `notify`, `utalk`, `Partners_files`
