...
Webhook para o **sistema externo** enviar atualizações de pedidos à instalação (sync manual / lote).

**Credenciais:** as mesmas de **outbound** (`client_id` / `client_secret` gerados no Admin → colar no External App anteriores).
...
- `id` = id do pedido antes (`soucannabis_order_id`)

## Regras

- Só **atualiza** pedido que já existe
- Não cria nem apaga
...
`NOT_FOUND` = pedido ainda não existe.
