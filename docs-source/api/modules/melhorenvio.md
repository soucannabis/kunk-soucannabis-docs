...
`POST /shipment/calculate` devolve preço/prazo **por serviço** disponível para o CEP. O carrinho lista **todas** as opções retornadas (não só Correios/Sedex no produto), agrupadas por transportadora quando fizer sentido na UI.
...
Cotação multi-serviço. O alias `/correios-quote` permanece por compat, mas o carrinho usa `/quote` (ou a facade `/freight/quote`).
...
- Histórico filtrava só `company.id === 1` (Correios). **Não restringe a Correios** — devolve Azul, Jadlog, etc., quando a API retornar.
...
Default: **desligado** — quem gera etiqueta é a Loggi.
