...
> Integra a instalação com a API externa do Kunk SouCannabis.
...
| `PATCH/DELETE/GET` | `/outbound/orders/:external_id` | SC → instalação local |
| `GET` | `/outbound/audit` | Exporta log de auditoria SC ↔ local |
...
| `POST` | `/webhooks/orders/sync` | Sync manual de pedidos (entrada →) |
| `GET` | `/webhook-info` | Admin: URLs e exemplo (sessão) |

### Auditoria Outbound

Toda mutação SC ↔ local (PATCH outbound, webhook sync, create/mirror/delete local → SC, recipient Pagarme) grava em `soucannabis_orders_audit`.
...
Obrigatório. Falha remota: log + `soucannabis_sync_error`; não desfaz escrita local (retry conforme política da instalação).
