...
> Cotação e etiquetas Loggi na API.
> Declaração de conteúdo: usar `store.freight.content_declaration` (Loja, compartilhada) — não gerar conteúdo aleatório.
...
Auth oficial: OAuth 2.0 **client credentials** → `POST https://api.loggi.com/v2/oauth2/token` (credenciais via config/env, nunca hardcoded).
...
Compatibilidade: respostas antigas devolviam um único `price`/`serviceLabel`. O usa `options[]`; o front escolhe a linha (favorita admin ou operador).
...
Upstream: cancel package Loggi; limpa `tracking_code` no order; status → `Pagamento concluído` .
...
|---|---|
| client_id/secret | credentials / env |
| Origem do frete | **`store.ship_from`** no admin Loja |
| Dimensões/peso | **Só** `store.freight.package` + `store.ship_from` no admin (sem fallback numérico) |
| Declaração de conteúdo | `store.freight.content_declaration` (compartilhada) |
| Valor declarado | `store.freight.content_declaration.total_value` |
| Cotações no carrinho | Devolver `options[]` (Econômico + Expresso) |
| Catálogo de serviços | `GET /service-options` a partir do enum + SISUs config |
