...
Substitui `files` + junctions. Tabela `files` no schema do produto + `orders_files`, `users_files`, `services_files`.
...
Drivers: `local`, `s3`, `gcs`. Configuração e admin: [files-cloud-storage.md](./files-cloud-storage.md).

| Env | Descrição |
|---|---|
| `FILES_DRIVER` | `local` \| `s3` \| `gcs` |
| `FILES_LOCAL_PATH` | path local (quando `local`) |
