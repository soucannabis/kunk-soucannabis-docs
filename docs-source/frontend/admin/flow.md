...
Administração do schema e relações do produto na própria interface.

### Comportamento

1. **Índice de collections** — lista a whitelist da API (`users`, `orders`, `system_users`, …), não tabelas fora do produto.
...
Fonte de verdade das relações: schema do produto e FKs documentadas na API (`kunk-api`).
