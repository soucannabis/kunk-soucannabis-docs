...
> Domínio **nativo** (não `/modules/*`). Substitui as rotas de compatibilidade `/terms/*`.
...
| Método | Path | Descrição |
|---|---|---|
| GET | `/doc-sign/status` | `{ status: "ready" }` quando o módulo está ativo |
| GET | `/terms/status` | **Alias deprecado** → mesmo handler |
| POST | `/terms/contracts` | **Alias deprecado** → `POST /doc-sign/contracts` |

Com doc-sign ativo, as rotas `/terms/*` deixam de responder `TERMS_MODULE_IN_DEVELOPMENT`.
