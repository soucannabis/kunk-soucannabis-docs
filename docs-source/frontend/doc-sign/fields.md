...
> Variáveis de preenchimento alinhadas ao schema do produto (`users`).
> Fonte canônica dos campos: [`../cadastramento/fields.md`](../cadastramento/fields.md).
...
> **CEP:** o schema do produto usa `cep`. Não há coluna `street_code` / `street-code`. A variável do termo é `{{cep}}`.
...
| Campo | Uso |
|---|---|
| `adhesion_term` | **UUID** FK lógica → `term_contracts.id`  |
