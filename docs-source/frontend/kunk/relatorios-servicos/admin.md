...
Taxas por tipo, preço padrão e se a doação reduz o valor a pagar são configuráveis por associação.
Defaults do produto: `association_fee = 0`, `default_consultation_price = null`, doação não desconta.
...
4. Envia e-mail **quando SMTP estiver configurado**; senão: **copiar link**
...
| Item | Motivo |
|---|---|
| Relatório de pedidos | Fora desta área |
| Integração pagamento / split | Módulo de pagamentos separado |
| Mensagens de chat ao contestar | Módulo externo (quando configurado) |
...
`contest_reports` já existe em `professionals` no schema do produto.
