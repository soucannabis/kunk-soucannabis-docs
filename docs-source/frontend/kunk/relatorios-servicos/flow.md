...
Linha do profissional (is_collaborator)
 ...
 → módulo e-mail indisponível: UI "Copiar link"
 ...
 → Qualquer `/app/*` fora do portal → bloqueado
```
...
Envio de e-mail do convite depende do módulo SMTP (`email_sent` / `email_status` no response da API).
...
| Sem profissional | Por mês e por profissional: contagem + soma (sempre a fórmula de `payable` — ver [fields.md](./fields.md)) |
...
| Pagamento | Ação de pagamento externo — não faz parte desta tela |
