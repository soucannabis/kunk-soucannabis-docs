...
**Não aceitar:** `coupon_id`, `no_commission`, `bvid` (campo reservado, não usado).
**Aceitar** `discount` e `donation` (campos manuais do carrinho).

Server deve:

1. Recalcular:
 - `products = Σ (item.amount × item.quantity)` — `amount` = preço unitário; **sem** re-lookup de catálogo
...
6. Triagem: após create, completar reception aberta do e-mail com `completion_reason: Pedido` (contrato da triagem).
...
**Facade de frete:** um único request devolve Loggi + Melhor Envio já normalizados (`option_key`, price, eta). O front não mergeia `/modules/loggi/…` + `/modules/melhorenvio/…`.
