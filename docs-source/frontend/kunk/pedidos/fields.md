...
| `discount` | Desconto manual (R$) — campo do carrinho |
| `donation` | Doação (R$) — campo do carrinho (reduz o total; **sem** cupons) |
...
### Campos **não** usados no checkout

| Campo | Motivo |
|---|---|
| `coupon_id` / cupons | Não disponível (doação manual permanece) |
| `no_commission` | Não disponível |
| `bvid` | Não usado neste produto |
...
Complementos: `freight_carrier` (`loggi` \| `melhorenvio`). Sem auto-tag pelo frete.
---
...
Total do pedido no server: `Σ (amount × quantity)` — sem re-lookup de catálogo.
...
|---|---|---|---|
| `store.freight.apply_to_total` | `boolean` | **`true`** | Se `true`, o valor do frete cotado entra no `total` do carrinho. Se `false`, simula mas não soma (comportamento). |
...
Dimensões e peso vêm **somente** da config do admin.

### `store.ship_from` — quem envia (admin Loja)

**Obrigatório.** Origem do frete (cidade, CNPJ, telefone, CEP) vem da config do admin.
...
Regras: obrigatório para etiqueta; cotação usa `total_value`; sem aleatório; snapshot `dce` no create-label; UI em `/servicos-externos/envio`.
