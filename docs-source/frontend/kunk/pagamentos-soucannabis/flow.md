...
3. Recebedor SouCannabis (via API)
 ...
 → cria recipient na conta Pagarme da associação
...
```text
local → SC : POST (após pago) / PATCH / DELETE
SC → local : PATCH/DELETE/GET …/outbound/orders/:external_id
SC → local : POST …/outbound/pagarme/recipients (cadastro recebedor)
SC → local : resolve associado via user_code quando a SC consultar
```
