...
## Tabela `reception` (schema do produto)

Colunas relevantes (snake_case).
...
| `completion_reason` | `Pedido` \| `Serviço` \| `Agendamento` |
| `is_prescriber` | Flags auxiliares |
| `chat_id` | Usado quando o módulo Utalk está ativo; caso contrário pode permanecer null |

Campos não usados neste produto: `bvid`.

---

## `system = triage`

Toda configuração da feature usa `system_configs` com `system = 'triage'`, `is_sensitive = false` (exceto secrets quando aplicável). Cascata: DB → env → `hardcoded_default`.
...
Opcional: `triage.public_form_title`, `triage.public_form_intro` (textos do form).
...
Não criar coluna SQL por campo custom — campos personalizados ficam em JSON.
