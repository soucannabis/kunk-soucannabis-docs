...
3. **Sidebar lateral de status** (configurados no admin):
 - Cada status mostra **contagem** dos registros com aquele status (no produto).
...
5. Busca textual (nome, e-mail, telefone, código) e filtros auxiliares (ex.: atendente), sem depender de chat ou afiliados externos.
...
| Com `associate_code` | Navega para pedido/serviço com query `u={associate_code}` (padrão) |

### Contabilização (no produto)
...
- Pode PATCH `completion_reason=Agendamento` (ou `Serviço`) + status Concluído **no momento do redirect**, se o produto mantiver esse comportamento.
...
- Quando **habilitado**: abrir painel/modal do associado (dados + documentos), **sem** aba de histórico de doações.

## 6. O que não entra no fluxo

- Chat externo como eixo obrigatório da fila (Utalk é módulo opcional)
- Match de afiliado / `bvid`
