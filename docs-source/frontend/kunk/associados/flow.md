...
`createAssociate.jsx` (só e-mail no painel). O funil completo continua no app `registration`.
...
Abrir modal (avatar / clique / ?a=)
 ...
 · menu Termo (Novo Termo / Copiar link) — ações mostram indisponibilidade enquanto o módulo de termos não estiver ativo
 Abas:
 0 Dados Pessoais
 1 Pacientes
 2 Prescritor
...
- Salvar → PATCH user
- Excluir → só se **sem** pedidos, serviços nem pacientes vinculados (**409** caso contrário)
...
Comportamento atual (módulo indisponível):
 · Qualquer ação → no-op / "Módulo de termos indisponível"
...
Observações/agenda usam responsável + paciente via `patient_user_code` no responsável quando aplicável.
...
Resumo: resultado associado abre `/app/acolhimento/associados?a={code}` (nova aba, como no produto) e oferece ação **Triagem**.

---

## 5. Status / fases na lista

Mapear para labels de UI (cards + filtros + coluna Status). Fonte de verdade:
