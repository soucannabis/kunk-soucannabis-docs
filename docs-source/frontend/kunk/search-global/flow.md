...
Se o hit for responsável com paciente(s) no funil, opcional mostrar paciente vinculado (lista via `patients` / primeiro paciente se necessário — **sem** depender de ativo global).
...
| Serviços | Nova aba `/app/acolhimento/servicos?s={associate_name}&h={ISO date}` | Filtra/destaca serviço na lista (como no produto) |
| Pedidos | Nova aba URL de pedidos com `?p={order_code}` (helper equivalente a `buildOrdersPageUrl`) | — |
| Triagem | Abrir fila com `?t={code}` ou `/app/acolhimento/triagem?t=` | Desabilitar se sem `code` |

Deep-link de operador: fila autenticada `/app/acolhimento/triagem?t=`. Manter `/triagem?t=` público só se a instância expuser o form público.
