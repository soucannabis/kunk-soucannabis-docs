...
O acesso a Serviços (e demais páginas do Kunk) é controlado por `role_pages` no admin, não só pela role na feature.
...
Spec cruzada: [`../associados/flow.md`](../associados/flow.md) §3.
...
- Se relacionar a um serviço existente **sem** profissional preenchido → PATCH no primeiro registro do grupo em vez de criar linha nova (comportamento).
...
Manter coluna e toggle/filtro de **status de pagamento** (`Aguardando Pagamento` | `Pagamento Concluído`) no produto — **sem** integração de checkout: o operador marca manualmente.
...
Detalhe do modal: ver fluxo do modal de infos do serviço acima.
