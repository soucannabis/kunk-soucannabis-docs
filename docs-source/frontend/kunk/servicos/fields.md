...
Nomes (histórico entre parênteses quando diferente).
...
| `associate_user_code` | varchar | sim | **Sempre o responsável** |
...
| `consultation_date` | timestamptz | não | Data/hora do atendimento |
| `price` | numeric | não | Valor da consulta (default do profissional) |
| `donation` | numeric | não | Doação |
...
| `created_by_user_code` | varchar | não | Operador que criou |
| `date_created` | timestamptz | sim | |

### Não disponível neste produto (não usar na UI / não gerar)
...
Filtro/toggle pago vs pendente na barra (no produto) **permanece**.
...
| `id` | serial | PK | |
| `name` | varchar | sim | |
| `last_name` | varchar | não |; histórico `lastname` |
| `email` | varchar | não | |
| `phone` | varchar | não | |
| `cpf` | varchar | não | |
...
| `services_description` | varchar | não | Descrição |
...
**Tipos de flag no schema:** `boolean` (`true`/`false`).
...
UI de create/edit: select obrigatório. Um profissional = **um** tipo principal (no produto).
...
Seed: tipos canônicos com `default_consultation_price: null` e `association_fee: 0` (sem hardcode 240/110 no produto). Instâncias (ex. Sou Cannabis) configuram no admin.
...
`isCollaboratorTrue`: aceita `true` / `1`.
